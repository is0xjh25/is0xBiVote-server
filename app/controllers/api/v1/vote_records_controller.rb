class Api::V1::VoteRecordsController < ApplicationController
	...
	def info
		...
		if vote_record
			return render json: { vote: VoteSerializer.new(vote), vote_record: VoteRecordSerializer.new(vote_record), post: PostCollectSerializer.new(vote, user_id: current_user.id) }, status: :ok
		else
